...
export default function ConsultationsPage() {
  ...
  try {
    await fetchFromApi('/call/queue/batch', {
      method: 'POST',
    ...
    try {
      ...
      alert(`[API] ${selectedIndividualsForConsultation.size}명 대기열 등록 후 상담 시작 요청 완료.`)
    } catch (startErr) {
      console.warn('Optional /call/start 호출 실패 또는 미구현:', startErr)
      alert(`[API] 대기열 등록은 완료되었습니다. 서버에서 자동으로 처리되거나, 별도 시작 트리거가 필요합니다.`)
    }
    ...
    setLiveStatuses(prev => {
      const clone = { ...prev }
      Array.from(selectedIndividualsForConsultation).forEach(id => { delete clone[id] })
      return clone
    })
    console.error("대기열 등록 실패:", err)
    alert("대기열 등록에 실패했습니다. 콘솔을 확인해주세요.")
  }
}
