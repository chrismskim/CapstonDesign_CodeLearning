...
export interface QuestionSet {
  ...
  // Add other fields from your QuestionSet entity if needed
}
...
export interface CallLog {
  ...
  summary: string
  result: number // 0: 상담 불가, 1: 상담 양호, 2: 심층 상담 필요
  fail_code: number
  need_human: number // 0: 아니오, 1: 상담 요청, 2: 위험 감지
...
export interface ConsultationStatusDto {
  vulnerableId: string;
...
interface DesireDetail {
  desire_index_list: number[]
  content: string
}

interface VulnerabilityInfo {
  risk_list: RiskDetail[]
  desire_list: DesireDetail[]
  risk_index_count?: Record<string, number>
  desire_index_count?: Record<string, number>
}
