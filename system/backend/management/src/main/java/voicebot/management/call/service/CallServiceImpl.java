...
public class CallServiceImpl implements CallService {
    ...
    private static final long QUESTION_CACHE_TTL = 1; // 1 hour
    ...
    public List<ConsultationStatusDto> getQueueStatus() {
        ...
        return items.stream()
                .map(obj -> {
                    ...
                    QuestionSet questionSet = (QuestionSet) redisTemplate.opsForValue().get(QUESTION_CACHE_PREFIX + item.getQuestionsId());
                    if (questionSet == null) {
                        // Fallback to DB if not in cache
                        questionSet = questionSetRepository.findById(item.getQuestionsId()).orElse(null);
                    }

                    if (vulnerable == null || questionSet == null) {
                        log.warn("Could not find Vulnerable or QuestionSet for queue item: {}", item.getQueueId());
                        return null; // Or create a DTO with an error state
                    }
                    ...
                    return new ConsultationStatusDto(
                            ...
                            item.getState(),
                            null // Error message is null for items in queue
                    );
                })
    ...
    public void startNextConsultation() {
        ...
        QuestionSet initialQuestionSet = (QuestionSet) redisTemplate.opsForValue().get(QUESTION_CACHE_PREFIX + item.getQuestionsId());
        if (initialQuestionSet == null) {
            initialQuestionSet = questionSetRepository.findById(item.getQuestionsId())
                    .orElseThrow(() -> {
                        log.error("QuestionSet not found in cache or DB for ID: {}. Skipping consultation.", item.getQuestionsId());
                        return new IllegalStateException("QuestionSet not found");
                    });
        }
        final QuestionSet questionSet = initialQuestionSet;

        // --- Session Index (회차) Management Logic ---
        ...
        // MonitoringService에 맞는 DTO로 변환하여 전송
        monitoringService.sendUpdate(new ConsultationStatusDto(
                item.getVulnerableId(),
                vulnerable.getName(), // vulnerable 객체에서 이름 가져오기
                questionSet.getTitle(), // questionSet 객체에서 제목 가져오기
                "IN_PROGRESS",
                null
        ));
        ...
        // Call the orchestrator API asynchronously
        ...
        webClient.post()
                ...
                .doOnSuccess(consultationResult -> {
                    ...
                    consultationResult.setQuestionSetId(item.getQuestionsId());
    ...
    private Object createOrchestratorRequest(Vulnerable vulnerable, QuestionSet questionSet, int sessionIndex) {
        // Create request body based on design document
