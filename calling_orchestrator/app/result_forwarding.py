...
# [수정] Spring Boot Controller 경로와 일치시킴 (/api/call/send_llm_result)
...
async def send_llm_result(llm_result: dict):
    ...
    print(f"[INFO] Sending result to Spring Boot ({SPRING_BOOT_URL}): {llm_result.get('user_phone')}")
    ...
    try:
        ...
        print(f"[ERROR] Failed to send result to Spring Boot: {e}")
